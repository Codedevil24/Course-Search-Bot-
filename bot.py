...
async def _run_bot():
    ...
    application.add_handler(CommandHandler('saved', h.saved_courses))
    application.add_handler(CommandHandler('new', h.new_courses))
    application.add_handler(CommandHandler('trending', h.trending_courses))
    application.add_handler(CommandHandler('admin', h.admin_panel))
    application.add_handler(CommandHandler('requests', h.requests_admin))
    application.add_handler(CommandHandler('requestdone', h.requestdone_admin))
    application.add_handler(CommandHandler('broadcast', h.broadcast_admin))
    application.add_handler(CommandHandler('maintenance', h.maintenance_admin))
    application.add_handler(CommandHandler('addcourse', h.addcourse))
    application.add_handler(CommandHandler('updatecourse', h.updatecourse))
    application.add_handler(CommandHandler('restorecourse', h.restorecourse))
    application.add_handler(CommandHandler('setthumb', h.setthumb))
    ...
    application.add_handler(CommandHandler('pendingpayments', h.pendingpayments))

    application.add_handler(CallbackQueryHandler(h.button_handler))
    application.add_handler(InlineQueryHandler(h.inline_query))
    application.add_handler(MessageHandler(filters.Document.ALL, h.importcsv_file))
...
def start_bot_in_background():
    ...
    if _bot_started:
        logger.info('Bot already started, skipping duplicate startup.')
        return
    _bot_started = True
    ...
    _bot_thread = threading.Thread(target=target, daemon=True, name='telegram-bot-thread')
    _bot_thread.start()
